...
// Полные правила соседства согласно требованиям
var adjacencyRules = map[int]map[int]bool{
	1:  {1: true, 3: true, 4: true, 5: true, 6: true, 7: true, 8: true, 9: true, 10: true},          // 1 может быть рядом с...
	2:  {2: true, 3: true},                                                                          // 2 только с 2 и 3
	3:  {1: true, 2: true, 3: true, 4: true, 5: true, 6: true, 7: true, 8: true, 9: true, 10: true}, // 3 со всеми
	4:  {1: true, 4: true, 5: true, 8: true},                                                        // 4 рядом с 1, 4, 5, 8
	5:  {4: true, 5: true, 10: true},                                                                // 5 рядом с 4, 5, 10
	6:  {6: true, 7: true},                                                                          // 6 рядом с 6, 7
	7:  {6: true, 7: true, 9: true},                                                                 // 7 рядом с 6, 7, 9
	8:  {1: true, 4: true, 8: true},                                                                 // 8 рядом с 1, 4, 8
	9:  {10: true},                                                                                  // 9 рядом с 10
	10: {5: true, 9: true, 10: true},                                                                // 10 рядом с 5, 9, 10
}
...
func (g *Game) generateInitialSites(count int) {
	...
	// Добавляем больше точек для 1-3
	for i := 10; i < count; i++ {
...
func (g *Game) precompute() {
	...
	// Этап 2: WFC с полными правилами
	g.applyOptimizedWFC()
	...
	for iter := 0; iter < wfcIterations; iter++ {
		for y := 1; y < screenHeight-1; y++ {
			for x := 1; x < screenWidth-1; x++ {
				...
				rules, exists := adjacencyRules[current]
				if !exists {
					continue
				}
...
func (g *Game) findBestReplacement(x, y int) int {
	// Собираем статистику по соседям
	...
	for dy := -1; dy <= 1; dy++ {
		for dx := -1; dx <= 1; dx++ {
			...
			if nx >= 0 && ny >= 0 && nx < screenWidth && ny < screenHeight {
				neighbor := g.wfcGrid[ny][nx]
				neighborCount[neighbor]++
			}
		}
	}

	// Ищем цифру с максимальной совместимостью
	bestDigit := 3 // Универсальный вариант
	maxScore := -1

	for digit := 1; digit <= 10; digit++ {
		score := 0
		rules, exists := adjacencyRules[digit]
		if !exists {
			continue
		}

		for neighbor, count := range neighborCount {
			if rules[neighbor] {
				score += count
			} else {
				score -= 10 // Штраф за несовместимость
			}
		}
...
func main() {
	...
	ebiten.SetWindowTitle("Voronoi-WFC with Full Rules")
